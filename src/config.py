from os import path
FILE_CONFIG = dict()
FILE_CONFIG['root'] = '..'
FILE_CONFIG['data'] = path.join(FILE_CONFIG['root'], 'data')
FILE_CONFIG['_oldaccounting'] = path.join(FILE_CONFIG['data'], 'finance')
# FILE_CONFIG['management'] = path.join(FILE_CONFIG['data'], 'management')
# FILE_CONFIG['service_charges'] = path.join(FILE_CONFIG['_oldaccounting'], 'service_charges')
FILE_CONFIG['blueprints'] = path.join(FILE_CONFIG['data'], 'blueprints')
FILE_CONFIG['imports'] = path.join(FILE_CONFIG['data'], 'imports')
FILE_CONFIG['reports'] = path.join(FILE_CONFIG['data'], 'reports')
