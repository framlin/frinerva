...
class TablePresenter {
    ...
    static create_editable_table(title, rows, properties, redraw) {
        ...
        rows.forEach((values) => {
            TablePresenter._add_booking_entries_row(table_elem, values, properties, redraw);
        });
    ...
    static _add_booking_entries_row(table, values, properties, redraw) {
        ...
        properties.forEach((prop, i) => {
            TablePresenter._insert_editable_cell(row, i, prop, values, redraw);
        });
    }

    static _insert_editable_cell(row, i, prop, buffer, redraw) {
        let {cell, text} = TablePresenter._add_cell(row, i, prop, buffer);
        cell.appendChild(text);
        cell.addEventListener('click', (event) => {
            let input_elem = TablePresenter._make_cell_editable(cell, row);
            TablePresenter._add_event_listener(input_elem, buffer, cell, redraw);
        });
    }
